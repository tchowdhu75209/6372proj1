...
#  k  RMSE      Rsquared   MAE     
#  5  2.374279  0.9291014  1.463024
#  7  2.816553  0.9016194  1.780684
#  9  3.485628  0.8617998  2.377731
...
#filterData = cleanData %>% filter(grepl("Developing", Status) | grepl("Developed", Status))
filterData$Status[grepl("Developing", filterData$Status)] = "Developing"
filterData$Status[grepl("Developed", filterData$Status)] = "Developed"
filterData$Status = as.factor(filterData$Status)

str(filterData)

set.seed(222)
trainIndices =  sample(seq(1:length(filterData$Status)), round(.7*length(filterData$Status)))
trainLED = filterData[trainIndices,]
testLED = filterData[-trainIndices,]


classifications = knn(trainLED[,c(4,5)],testLED[,c(4,5)],trainLED$Status, prob = TRUE, k = 5)
table(testLED$Status,classifications)
CM = confusionMatrix(table(testLED$Status,classifications))
CM
